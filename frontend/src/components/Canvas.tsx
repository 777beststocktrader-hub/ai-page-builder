...
function DashboardStart({
  ...
  const steps = [
    ...
    { n: '3', title: 'Edit and publish', body: 'Click any section to edit, preview on mobile, then push to Shopify.', icon: <Zap size={18} /> },
  ];

  return (
    <div className="flex-1 overflow-y-auto bg-gradient-to-br from-slate-50 via-white to-indigo-50/40 cursor-default">
      {/* Hero */}
      <div className="relative overflow-hidden bg-slate-950 px-6 py-14 sm:px-10 sm:py-20">
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top_left,#3730a355_0%,transparent_60%)]" />
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_bottom_right,#0e7490_0%,transparent_55%)] opacity-20" />
        <div className="relative mx-auto max-w-3xl text-center">
          <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-indigo-400/25 bg-indigo-500/10 px-4 py-1.5 text-xs font-bold uppercase tracking-widest text-indigo-300">
            <Sparkles size={11} />
            AI Shopify Page Builder
          </div>
          <h1 className="text-4xl font-black leading-[1.05] tracking-tight text-white sm:text-5xl lg:text-6xl">
            One product.<br />
            <span className="bg-gradient-to-r from-cyan-400 to-indigo-400 bg-clip-text text-transparent">A page that sells it.</span>
          </h1>
          ...
          <div className="mt-8 flex flex-col items-center gap-3 sm:flex-row sm:justify-center">
            ...
            <button
              onClick={onTryDemo}
              disabled={generating}
              className="inline-flex items-center gap-2 rounded-2xl border border-white/15 bg-white/8 px-6 py-3.5 text-sm font-bold text-slate-300 transition-all hover:bg-white/12 hover:text-white disabled:opacity-60"
            >
      ...
      <div className="mx-auto max-w-4xl px-6 py-10 space-y-8">
        ...
        <div className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
          <div className="flex items-center gap-2 border-b border-slate-100 bg-slate-50 px-5 py-3">
            ...
            <span className="ml-3 flex-1 rounded-full bg-white border border-slate-200 px-3 py-1 text-xs text-slate-400">yourstore.myshopify.com/pages/product-landing</span>
            <span className="rounded-full bg-emerald-50 px-2.5 py-1 text-[10px] font-black text-emerald-700 border border-emerald-200">Published</span>
          </div>
          <div className="p-6">
            <div className="grid gap-4 sm:grid-cols-[1fr_200px]">
              <div>
                <div className="mb-2 h-2.5 w-24 rounded-full bg-indigo-100" />
                <div className="mb-2 h-7 w-4/5 rounded-lg bg-slate-900" />
                <div className="mb-1 h-3 w-full rounded-full bg-slate-200" />
                <div className="mb-4 h-3 w-3/4 rounded-full bg-slate-200" />
                <div className="flex gap-2 mb-5">
                  <div className="h-10 w-32 rounded-xl bg-indigo-600" />
                  <div className="h-10 w-28 rounded-xl border border-slate-200 bg-white" />
                </div>
              ...
              <div className="rounded-2xl bg-slate-50 border border-slate-200 p-3">
                <img src={SAMPLE_PRODUCT.imageUrl} alt="Product" className="w-full aspect-square rounded-xl object-cover" />
                <div className="mt-3 space-y-1.5">
                  <div className="flex gap-0.5">
                    {[...Array(5)].map((_, i) => <Star key={i} size={10} fill="#facc15" className="text-yellow-400" />)}
                    <span className="ml-1 text-[10px] text-slate-500">4.9 (700+)</span>
                  </div>
                  <div className="h-3 w-4/5 rounded-full bg-slate-900" />
                  <div className="h-8 w-full rounded-lg bg-indigo-600" />
                </div>
              </div>
            </div>
            <div className="mt-4 grid grid-cols-4 gap-2 border-t border-slate-100 pt-4">
              {['Hero', 'Benefits', 'Reviews ×15', 'FAQ + CTA'].map((label) => (
                <div key={label} className="rounded-lg bg-slate-50 border border-slate-200 px-2 py-1.5 text-center">
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          <p className="text-xs font-black uppercase tracking-widest text-slate-400 mb-1">No Shopify product yet?</p>
          <h3 className="text-base font-black text-slate-950">Generate from a description</h3>
          <p className="mt-1 mb-4 text-sm text-slate-500">Type what you're selling and AI will build a full page.</p>
          <div className="flex gap-2">
            <input
              ...
              placeholder='e.g. "portable neck fan for summer travel"'
              className="flex-1 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-700 outline-none placeholder-slate-400 focus:border-indigo-400 focus:bg-white transition-all"
              disabled={generating}
            />
            <button
              onClick={() => onGenerate()}
              disabled={generating || !goal.trim()}
              className="inline-flex items-center gap-2 rounded-xl bg-slate-950 px-5 py-3 text-sm font-black text-white hover:bg-slate-800 disabled:opacity-50 disabled:cursor-not-allowed transition-all"
            >
